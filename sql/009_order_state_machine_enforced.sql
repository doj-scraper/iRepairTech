...
-- =========================================
-- Production-grade state transitions with DB-level enforcement

-- 1. Extended enum type
...
-- 2. Extend orders table with state tracking
ALTER TABLE public.orders 
DROP COLUMN IF EXISTS status,
ADD COLUMN status order_status NOT NULL DEFAULT 'pending';

-- 3. Transition rules table (graph definition)
...
-- 5. State history table (audit trail)
...
-- 6. Enforced transition function (ONLY write path)
