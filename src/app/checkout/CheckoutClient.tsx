...
import { ArrowRight, FileCheck2, ShieldCheck, Truck, Warehouse } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchQuote = async () => {
      ...
      try {
        ...
        const total = nextQuotedItems.reduce((sum, item) => sum + item.price_cents * item.quantity, 0);
      ...
      <main id="main-content" className="px-4 py-8 md:px-6 md:py-12">
        <div className="container space-y-8">
          <section className="shell-frame overflow-hidden">
            <div className="shell-core grid gap-8 px-5 py-6 sm:px-6 sm:py-8 lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:px-10 lg:py-10">
                  ...
                  {[
                    {
                      ...
                      body: 'Live prices are re-quoted from Supabase before checkout starts.',
                    },
                    {
                      icon: FileCheck2,
                      title: 'Terms captured',
                      body: 'Accepted wholesale terms are recorded with the draft order.',
                    },
                    {
                      icon: Truck,
                      title: 'Fulfillment continuity',
                      body: 'The buyer moves from checkout to a clean confirmation flow.',
                    },
                  ].map((item) => (
              ...
              <div className="space-y-4">
                <div className="relative overflow-hidden rounded-[2rem] border border-border/70 bg-primary text-primary-foreground shadow-elegant">
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(184,138,59,0.24),transparent_32%),linear-gradient(180deg,rgba(255,255,255,0.1),transparent_48%)]" />
                  <Image
                    src="/iphone-screen-incell.png"
                    alt="Wholesale checkout reference image"
                    width={960}
                    height={720}
                  ...
                  <div className="absolute inset-x-0 bottom-0 border-t border-white/10 bg-primary/70 p-5 backdrop-blur-sm">
                    <div className="grid gap-3 sm:grid-cols-3">
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.2em] text-white/65">Line items</p>
                        <p className="mt-1 text-base font-semibold">{quotedItems.length}</p>
                      </div>
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.2em] text-white/65">Units</p>
                        <p className="mt-1 text-base font-semibold">{summary.units}</p>
                      </div>
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.2em] text-white/65">Quote total</p>
                        <p className="mt-1 text-base font-semibold">
                          {quoteLoading ? 'Calculating…' : `$${(quotedTotalCents / 100).toFixed(2)}`}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-[1.5rem] border border-hairline/70 bg-secondary/35 p-4">
                    ...
                    <p className="mt-4 text-xs uppercase tracking-[0.18em] text-muted-foreground">Stockroom</p>
                    <p className="mt-2 text-sm text-primary">Houston-based inventory and order review for professional repair operators.</p>
                  </div>
                  <div className="rounded-[1.5rem] border border-hairline/70 bg-secondary/35 p-4">
                    <ShieldCheck className="h-5 w-5 text-accent" />
                    <p className="mt-4 text-xs uppercase tracking-[0.18em] text-muted-foreground">Terms</p>
                    <p className="mt-2 text-sm text-primary">Wholesale purchasing terms are captured before payment is routed to Stripe.</p>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  {quoteLoading ? (
                    ...
                    quotedItems.map((item) => (
                      <div
                        ...
                        className="rounded-[1.4rem] border border-hairline/70 bg-secondary/35 p-5 sm:rounded-[1.6rem]"
                      >
            ...
            <div className="shell-frame">
              <div className="shell-core px-5 py-6 sm:px-6 sm:py-8 md:px-8 md:py-8">
                ...
                <div className="space-y-5">
                  ...
                  <Link
                    className="inline-flex items-center gap-2 text-sm font-medium text-accent transition hover:text-primary"
                    href="/shop/catalog"
                  >
                    Return to catalog
